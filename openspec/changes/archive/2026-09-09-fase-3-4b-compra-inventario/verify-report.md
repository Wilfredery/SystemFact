...
### Findings closed (remediation batch, request-id unblocked-findings-1, attempt 1/3)
- W-1 (doc drift): `9369062` — design.md now carries a "Ratified decision: all-branch cost aggregate READ GUC narrowing" section (empresa GUC untouched, entry writes branch-bound, no-migration rationale + tradeoff); inventario spec reconciled with a single read-only exception clause so spec and design no longer contradict. Documentation-only.
- S-1 (missing DB test): `eed192b` — added a real-DB same-tenant wrong-branch receipt test to `compra-recibir.integration.test.ts`. OBSERVED behavior asserted: with RLS `compra_isolation` (empresa AND sucursal, no es_admin bypass), an Administrador session at branch A1 never sees the A2 purchase, so `recibirCompra` short-circuits to typed `COMPRA_NO_ENCONTRADA` before the application `COMPRA_SUCURSAL_INVALIDA` guard fires. The test documents this (RLS is the stricter outcome; the app guard is a defense-in-depth backstop) and proves zero side effects (state PENDIENTE, zero movements, no stock row, cost unchanged). No production change.
- S-2 (TS18048): `eed192b` — one-liner optional-chain narrowing in `compra-non-admin.integration.test.ts:87`; semantics identical; full-program `tsc --noEmit` now 0 errors.
- Re-run gates: unit 339/339, integration 44/44 (still 14 suites — the S-1 test was added to the existing `compra-recibir` suite, reusing its helpers), lint 0, tsc 0. Warnings/suggestions cleared; verdict remains PASS WITH WARNINGS with W-1/S-1/S-2 now closed. Ready for archive.
