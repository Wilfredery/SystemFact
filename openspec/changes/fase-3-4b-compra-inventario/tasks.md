...
- [x] 1.1 RED unit `app/src/modules/inventario/domain/`: cost function `calcularNuevoCostoPromedio` (Decimal half-up 2dp) — R1 mixed ITBIS 18/16/0 with ITBIS-exclusive costs; R2 all-branch denominator (10@A + 40@B, receive 50 @ 100, CP 80 ⇒ 86.67). Pure, no DB. *(Note: the `86.67` in the spec/task example is arithmetically inconsistent with the normative formula — the correct all-branch result for those inputs is `90.00`; the single-branch defect is `96.67`. Implemented per the formula and flagged for spec reconciliation; see apply-progress.)*
