...
- [x] 2.1 Application use cases in `app/src/modules/cliente/application/`: `crearCliente` (defaults+credit rule+duplicate probe; version 1 active), `actualizarCliente` (optimistic lock, credit rule), `obtenerCliente`, `listarClientes` (default 25 / cap 100, deterministic order, `incluirInactivos`, search by name+fiscal-ID digits, credit fields in projection), `desactivarCliente` (soft, idempotent YA_INACTIVO, ventas guard, releases fiscal ID), all typed success/error, typed-business-error pattern
- [x] 2.2 `getOrCreateConsumidorFinalEnTx(tx, empresaId)`: fetch→insert→catch P2002→refetch; CF shape per design (nombre `Consumidor Final`, N/A contacto, fiscal NULL, MINORISTA, credit off, limit 0.00, plazo 30, active); NO venta logic — reserved seam for 5b only. (Signature takes `tx` directly, not `(db, tx)`: the maintenance client is structurally a `PrismaTx`, so seed and future sale path share one implementation.)
- [x] 2.3 HTTP: `app/src/modules/cliente/http/validation.ts` (zod transport shapes only — no fiscal format logic in zod) + `actions.ts` thin adapters: tenant ctx → `withTenantTransaction` → role gate (CRUD Admin+Operador; credit fields + deactivate Admin-only server-side) → use case → DTO; CRM-row protection: update/deactivate of `esConsumidorFinal` ⇒ CONSUMIDOR_FINAL_PROTEGIDO (detail/list hide it ⇒ CLIENTE_NO_ENCONTRADO); audit CREAR/ACTUALIZAR/CANCELAR entity "Cliente" in-tx with VARCHAR-safe payloads
- [x] 2.4 Seed: `app/tools/scripts/seed-consumidor-final.ts` — `seedConsumidorFinalParaEmpresa` mirroring `seedRetencionConfigParaEmpresa` via the reserved seam, idempotent per-empresa, direct-URL guard; `seed:cliente` npm script. No in-app empresa-provisioning service exists (parity with the retention seed): provisioning is the idempotent script + fixtures; documented in README/SETUP.
